...
public class RockMonsterAI : MonoBehaviour
{
    [SerializeField] Transform DestinationOne, DestinationTwo, playerModel, Eye, boulderSpawnPoint;
    ...
    Animator anim;
    Vector2 lastPos;
    public AudioSource rockThrowingSound;
    ...
    private void RockTossSound()
    {
        rockThrowingSound.Play();
    }
...
//===================== detected the player in view angle =========================
    ...
    private void ThrowBoulder() // animation event
    {
        if (newBoulder == null)
        {
            newBoulder = Instantiate(BoulderPrefab, boulderSpawnPoint.position, BoulderPrefab.transform.rotation);

            if (transform.eulerAngles.y == 0)
            {
                newBoulder.GetComponent<boulderScript>().Direction = -1;
